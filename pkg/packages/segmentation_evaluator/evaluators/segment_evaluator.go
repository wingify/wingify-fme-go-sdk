...
func (s *SegmentEvaluator) CheckLocationPreSegmentation(locationMap map[string]interface{}) bool {
	...
	if s.Context.Wingify == nil || s.Context.Wingify.Location == nil || len(s.Context.Wingify.Location) == 0 {
	...
	return segmentUtils.ValuesMatch(locationMap, s.Context.Wingify.Location)
}
...
func (s *SegmentEvaluator) CheckUserAgentParser(uaParserMap map[string][]string) bool {
	...
	if s.Context.Wingify == nil || s.Context.Wingify.GetUaInfo() == nil || len(s.Context.Wingify.GetUaInfo()) == 0 {
		if !s.ServiceContainer.GetSettingsManager().GetIsGatewayServiceProvided() {
	...
	return segmentUtils.CheckValuePresent(uaParserMap, s.Context.Wingify.GetUaInfo())
}
...
func (s *SegmentEvaluator) CheckInUserStorage(featureKey string, context *user.WingifyUserContext) (result bool) {
	defer func() {
		if r := recover(); r != nil {
