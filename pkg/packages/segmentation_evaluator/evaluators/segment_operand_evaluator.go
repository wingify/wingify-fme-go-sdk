...
import (
	...
	segmentUtils "github.com/wingify/wingify-fme-go-sdk/pkg/packages/segmentation_evaluator/utils"
)
...
func (s *SegmentOperandEvaluator) EvaluateUserDSL(dslOperandValue string, properties map[string]interface{}) bool {
	...
	for _, user := range users {
		...
		if wingifyUserID, exists := properties[constants.VariationTargetingUserIDKey]; exists {
			if userTrimmed == fmt.Sprint(wingifyUserID) {
				return true
			}
		}
	}
...
func (s *SegmentOperandEvaluator) EvaluateUserAgentDSL(dslOperandValue string, context *user.WingifyUserContext) bool {
...
func (s *SegmentOperandEvaluator) EvaluateStringOperandDSL(dslOperandValue string, context *user.WingifyUserContext, operandType enums.SegmentOperatorValue) bool {
...
func (s *SegmentOperandEvaluator) getTagValueForOperandType(context *user.WingifyUserContext, operandType enums.SegmentOperatorValue) string {
...
// getBrowserVersionFromContext extracts browser version from Wingify context
func (s *SegmentOperandEvaluator) getBrowserVersionFromContext(context *user.WingifyUserContext) string {
	if context == nil || context.GetWingify() == nil {
		return ""
	}
	ua := context.GetWingify().GetUaInfo()
	if len(ua) == 0 {
		return ""
	}
...
// getOsVersionFromContext extracts OS version from Wingify context
func (s *SegmentOperandEvaluator) getOsVersionFromContext(context *user.WingifyUserContext) string {
	if context == nil || context.GetWingify() == nil {
		return ""
	}
	ua := context.GetWingify().GetUaInfo()
