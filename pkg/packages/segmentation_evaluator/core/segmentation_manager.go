...
func (sm *SegmentationManager) SetContextualData(
	...
	context *user.WingifyUserContext, // UserContext object containing the user context
) {
	...
	if feature.IsGatewayServiceRequired && serviceContainer.GetSettingsManager().GetIsGatewayServiceProvided() {
		...
		wingifyData, err := gateway.GetFromGatewayService(serviceContainer, queryParams, constants.EndpointGetUserData)
		if err != nil {
			return
		}
		...
		if wingifyData != "" {
			...
			var gatewayServiceModel user.ContextWingify
			err := json.Unmarshal([]byte(wingifyData), &gatewayServiceModel)
			if err != nil {
			...
			context.Wingify = &gatewayServiceModel
		}
	}
}
