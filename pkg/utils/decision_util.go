...
import (
	...
	_ "github.com/wingify/wingify-fme-go-sdk/pkg/models/settings" // Used indirectly by GetGroupDetailsIfCampaignPartOfIt
	"github.com/wingify/wingify-fme-go-sdk/pkg/models/storage"
...
func CheckWhitelistingAndPreSeg(
	...
	wingifyUserID := GetUUID(context.ID, accountID)
	...
	if campaignModel.Type == string(enums.CampaignTypeAB) {
		...
		if campaignModel.IsUserListEnabled {
			variationTargetingVariables[constants.VariationTargetingUserIDKey] = wingifyUserID
		} else {
	...
	// Set _wingifyUserId for custom variables
	...
	if campaignModel.IsUserListEnabled {
		customVariables[constants.VariationTargetingUserIDKey] = wingifyUserID
	} else {
	...
	if groupId != "" && groupId != "null" {
		...
		if err == nil {
			...
			if exists && groupWinnerCampaignId != "" {
				...
				storedDataMap, err := storageService.GetDataInStorage(constants.WingifyMetaMegKey+strconv.Itoa(groupIdInt), context)
				if err != nil {
