...
import (
	...
	"github.com/wingify/wingify-fme-go-sdk/pkg/models/request"
...
func GetEventBasePayload(settingsManager interfaces.SettingsManagerInterface, userID string, eventName string, visitorUserAgent string, ipAddress string, usageStatsAccountId int) *request.EventArchPayload {
	...
	eventArchData := &request.EventArchData{
		...
		Event:     createEvent(settingsManager.GetSDKKey(), settingsManager.GetSDKName(), eventName),
...
func GetTrackUserPayloadData(serviceContainer interfaces.ServiceContainerInterface, eventName string, campaignID int, variationID int, context *user.WingifyUserContext) map[string]interface{} {
...
func GetAttributePayloadData(serviceContainer interfaces.ServiceContainerInterface, eventName string, attributeMap map[string]interface{}, context *user.WingifyUserContext) map[string]interface{} {
...
// SendPostAPIRequest sends visitor and conversion events to Wingify
func SendPostAPIRequest(serviceContainer interfaces.ServiceContainerInterface, properties map[string]string, payload map[string]interface{}, context *user.WingifyUserContext, campaignInfo map[string]interface{}) {
	...
	if eventName == enums.VariationShown.GetValue() {
	...
	} else if eventName == enums.SyncVisitorProp.GetValue() {
		apiName = string(enums.ApiSetAttribute)
		extraDataForMessage = apiName
	} else if eventName == enums.SDKInitEvent.GetValue() {
		apiName = eventName
		extraDataForMessage = apiName
	} else if eventName == enums.SDKUsageStats.GetValue() {
		apiName = eventName
		extraDataForMessage = apiName
	} else {
	...
	go func() {
		...
		if response != nil && response.TotalAttempts > 0 {
			...
			SendDebugEventToWingify(serviceContainer.GetSettingsManager(), debugEventProps)
		}
		if response != nil && response.StatusCode >= 200 && response.StatusCode < 300 {
			...
			if eventName != enums.DebuggerEvent.GetValue() && eventName != enums.SDKInitEvent.GetValue() && eventName != enums.SDKUsageStats.GetValue() {
...
func SendEventDirectlyToDACDN(settingsManager interfaces.SettingsManagerInterface, properties map[string]string, payload map[string]interface{}, eventName string) {
	...
	go func() {
		networkManager := settingsManager.GetNetworkManager()
		if networkManager == nil {
			return
		}
		response := networkManager.Post(request, nil)
		if response == nil || (response.StatusCode < 200 || response.StatusCode >= 300) {
			if eventName != enums.DebuggerEvent.GetValue() {
...
func GetSDKUsageStatsEventPayload(settingsManager interfaces.SettingsManagerInterface, userID string, eventName string, usageStatsAccountId int, usageStatsData map[string]interface{}) map[string]interface{} {
	...
	properties.D.Event.Props.Product = constants.FME
	properties.D.Event.Props.Meta = usageStatsData
...
func GetDebuggerEventPayload(settingsManager interfaces.SettingsManagerInterface, eventProps map[string]interface{}) map[string]interface{} {
	...
	properties := GetEventBasePayload(settingsManager, userID, enums.DebuggerEvent.GetValue(), "", "", 0)
	...
	eventProps[enums.DebugPropSDKName.GetValue()] = settingsManager.GetSDKName()
	...
	properties.D.Event.Props.Meta = eventProps
...
func createEvent(sdkKey string, sdkName string, eventName string) *request.Event {
	props := createProps(sdkKey, sdkName)
...
func createProps(sdkKey string, sdkName string) *request.Props {
...
func createVisitor(sdkKey string) *request.Visitor {
	visitorProps := map[string]interface{}{
		constants.FsEnvironment: sdkKey,
	}
