# Wingify Feature Management and Experimentation SDK for Go

[![CI](https://github.com/wingify/wingify-fme-go-sdk/workflows/CI/badge.svg?branch=master)](https://github.com/wingify/wingify-fme-go-sdk/actions?query=workflow%3ACI)
[![License](https://img.shields.io/badge/License-Apache%202.0-blue.svg)](http://www.apache.org/licenses/LICENSE-2.0)

## Overview

The **Wingify Feature Management and Experimentation SDK** (Wingify FME Go SDK) enables Go developers to integrate feature flagging and experimentation into their applications. This SDK provides full control over feature rollout, A/B testing, and event tracking, allowing teams to manage features dynamically and gain insights into user behavior.
...
Our [Build](https://github.com/wingify/wingify-fme-go-sdk/actions) is successful on these Go Versions -
...
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | -------- | ------------------------------- |
| `accountId`                  | Wingify Account ID for authentication.                                                                                                                          | Yes          | String/Int   | `"123456"` or `123456`                      |
...
| `pollInterval`               | Time interval for fetching updates from Wingify servers (in milliseconds).                                                                                      | No           | Number   | `60000`                         |
| `gatewayService`             | Configuration for integrating the Wingify Gateway Service.                                                                                   | No           | Object   | see [Gateway](#gateway) section |
...
Refer to the [official Wingify FME documentation](https://developers.vwo.com/v2/docs/fme-go-install) for additional parameter details.
...
flag, err := client.GetFlag("feature-key", context)
```

### FME Web Connectivity

FME Web Connectivity allows you to keep visitor identity and sessions in sync between the Go SDK and Wingify Web Insights.

#### Using web UUID from `context["id"]`

When **web connectivity is enabled**, if you pass a valid Wingify Web UUID in `context["id"]`, the Go SDK will **use it directly as the visitor UUID** instead of hashing/generating a new one.  
...
```go
client, err := wingify.Init(map[string]interface{}{
    "accountId": 123456,
...
flag1, err := client.GetFlag("feature-key", contextWithGeneratedUuid)
uuid1 := flag1.GetUUID()
fmt.Println("Visitor UUID (generated):", uuid1)

// Use your own UUID (e.g. from the Wingify Web SDK) by passing a valid web UUID in context.id
contextWithCustomUuid := map[string]interface{}{
    "id": "D7E2EAA667909A2DB8A6371FF0975C2A5", // your existing Wingify Web UUID
}
flag2, err := client.GetFlag("feature-key", contextWithCustomUuid)
uuid2 := flag2.GetUUID()
...
The Go SDK also exposes the session identifier via `GetSessionId()` on the flag result, which is useful for aligning sessions with the Wingify Web SDK or other systems:
...
flag, err := client.GetFlag("feature-key", contextWithSession)
if err != nil {
...
```go
featureFlag, err := client.GetFlag("feature_key", context)
if err != nil {
    log.Printf("Error getting feature flag: %v", err)
    return
}
...
trackResponse, err := client.TrackEvent("purchase", context, eventProperties)
if err != nil {
    log.Printf("Error tracking event: %v", err)
} else {
    fmt.Println("Event tracked:", trackResponse)
...
User attributes provide rich contextual information about users, enabling powerful personalization. The `SetAttribute()` method provides a simple way to associate these attributes with users in Wingify for advanced segmentation. The method accepts an attribute map and context map containing the user information.
...
The `pollInterval` is an optional parameter that allows the SDK to automatically fetch and update settings from Wingify servers at specified intervals. The polling interval can be configured in three ways:
...
2. Wingify Application Settings: If configured in your Wingify application settings, that interval will be used
...
Wingify FME by default logs all `ERROR` level messages to your server console.
To gain more control over logging behaviour, you can use the `logger` parameter in the `Init` configuration.

| **Parameter** | **Description** | **Required** | **Type** | **Default Value** |
|---------------|-----------------|--------------|----------|-------------------|
| `level` | Log level to control verbosity of logs | Yes | string | `"ERROR"` |
| `prefix` | Custom prefix for log messages | No | string | `"Wingify-SDK"` |
...
The Wingify FME Gateway Service is an optional but powerful component that enhances Wingify Feature Management and Experimentation (FME) SDKs. It acts as a critical intermediary for pre-segmentation capabilities based on user location and user agent (UA). By deploying this service within your infrastructure, you benefit from minimal latency and strengthened security for all FME operations.
...
The version history tracks changes, improvements, and bug fixes in each version. For a full history, see the [CHANGELOG.md](https://github.com/wingify/wingify-fme-go-sdk/blob/master/CHANGELOG.md).

## Contributing

We welcome contributions to improve this SDK! Please read our [contributing guidelines](https://github.com/wingify/wingify-fme-go-sdk/blob/master/CONTRIBUTING.md) before submitting a PR.

## Code of Conduct

Our [Code of Conduct](https://github.com/wingify/wingify-fme-go-sdk/blob/master/CODE_OF_CONDUCT.md) outlines expectations for all contributors and maintainers.

## License

[Apache License, Version 2.0](https://github.com/wingify/wingify-fme-go-sdk/blob/master/LICENSE)
