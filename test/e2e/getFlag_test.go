...
import (
	...
	"github.com/wingify/wingify-fme-go-sdk/test/data"
	"github.com/wingify/wingify-fme-go-sdk/test/data/testCases"
)
...
func runTests(t *testing.T, tests []testCases.TestData, useStorage bool, settingsMap map[string]string) {
	for _, testData := range tests {
		t.Run(testData.Description, func(t *testing.T) {
			...
			// Get feature flag
			featureFlag, err := wingifyClient.GetFlag(testData.FeatureKey, context)
			assert.NoError(t, err)
	...
	for _, testData := range tests {
		t.Run(testData.Description, func(t *testing.T) {
			...
			for _, userID := range testData.UserIds {
				...
				featureFlag2, err := wingifyClient.GetFlag(testData.FeatureKey2, context)
				assert.NoError(t, err)
