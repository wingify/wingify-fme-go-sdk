...
import (
	...
	"github.com/wingify/wingify-fme-go-sdk/pkg/api"
	"github.com/wingify/wingify-fme-go-sdk/pkg/constants"
	"github.com/wingify/wingify-fme-go-sdk/pkg/core"
...
// WingifyClient represents the Wingify FME client with API methods
type WingifyClient struct {
...
// newWingifyClient creates a new WingifyClient instance
func newWingifyClient(settings *settingsModel.Settings, wingifyBuilder *WingifyBuilder) *WingifyClient {
	// Create service container
	serviceContainer := core.NewServiceContainer(
		fmt.Sprintf("%d_%s", wingifyBuilder.options.AccountID, wingifyBuilder.options.SDKKey),
		wingifyBuilder.logManager,
		wingifyBuilder.settingsManager,
		wingifyBuilder.options,
		wingifyBuilder.batchEventQueue,
		settings,
		wingifyBuilder.networkManager,
	)

	// Validate settings once during initialization
	isSettingsValid, settingsInvalidReason := validateSettingsInternal(wingifyBuilder.originalSettings, settings, wingifyBuilder)
	if isSettingsValid {
		serviceContainer.GetSettingsManager().SetSettingsValidOnInit(true)
	}
	wingifyClient := &WingifyClient{
		wingifyBuilder:        wingifyBuilder,
		originalSettings:      wingifyBuilder.originalSettings,
	...
	if wingifyClient.wingifyBuilder.batchEventQueue.IsInitialized() {
		wingifyClient.wingifyBuilder.batchEventQueue.SetSettings(settings)
	}
	...
	utils.ProcessSettings(settings, wingifyBuilder.logManager)

	wingifyBuilder.logManager.Info(log.BuildMessage(log.InfoLogMessagesEnum["CLIENT_INITIALIZED"], map[string]interface{}{
		"sdkName":    wingifyBuilder.settingsManager.GetSDKName(),
		"sdkVersion": constants.SDKVersion,
	}))
	return wingifyClient
}

// sendInitAndUsageStatsEvents sends the SDK init and usage stats events to Wingify
func sendInitAndUsageStatsEvents(serviceContainer interfaces.ServiceContainerInterface) {
	// Convert context to WingifyUserContext
	contextModel := user.NewWingifyUserContext(map[string]interface{}{
	...
	settingsObj := serviceContainer.GetSettingsManager().GetSettingsObject()
	if settingsObj == nil {
		return
	}

	sdkMetaInfo := settingsObj.SDKMetaInfo
	...
	if serviceContainer.GetSettingsManager().GetIsSettingsValidOnInit() {
		...
		usageStatsAccountId := settingsObj.GetUsageStatsAccountID()
...
func (client *WingifyClient) GetFlag(featureKey string, context map[string]interface{}) (flag models.GetFlagResponse, err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			client.wingifyBuilder.logManager.Error("EXECUTION_FAILED", map[string]interface{}{
				"apiName": apiName,
	...
	client.wingifyBuilder.logManager.Debug(log.BuildMessage(log.DebugLogMessagesEnum["API_CALLED"], map[string]interface{}{
		"apiName": apiName,
	}))

	if context == nil || context[enums.ContextID.GetValue()] == nil || context[enums.ContextID.GetValue()] == "" {
		client.wingifyBuilder.logManager.Error("INVALID_CONTEXT", nil, map[string]interface{}{"an": apiName})
		return models.NewGetFlag(false, nil, "", sessionId), fmt.Errorf("invalid context")
	}

	if featureKey == "" {
		client.wingifyBuilder.logManager.Error("INVALID_PARAM", map[string]interface{}{
			"apiName":     apiName,
			"key":         "featureKey",
			"type":        "empty string",
	...
	if !client.isSettingsValid {
		client.wingifyBuilder.logManager.Error("INVALID_SETTINGS_SCHEMA", map[string]interface{}{
			"errors":    client.settingsInvalidReason,
			"accountId": strconv.Itoa(client.wingifyBuilder.options.AccountID),
			"sdkKey":    client.wingifyBuilder.options.SDKKey,
	...
	if seed, ok := context[enums.ContextBucketingSeed.GetValue()]; ok {
		if seedStr, isStr := seed.(string); !isStr || strings.TrimSpace(seedStr) == "" {
			client.wingifyBuilder.logManager.Error("INVALID_BUCKETING_SEED", nil, map[string]interface{}{"an": apiName})
			delete(context, enums.ContextBucketingSeed.GetValue())
		}
	}

	uuid, isWebUuid, err := utils.GetUUIDFromContext(context, string(apiName), client.wingifyBuilder.options.AccountID, client.settings)
	if err != nil {
		client.wingifyBuilder.logManager.Error("EXECUTION_FAILED", map[string]interface{}{
			"apiName": apiName,
			"err":     err.Error(),
	...
	if isWebUuid {
		client.wingifyBuilder.logManager.Debug(log.BuildMessage(log.DebugLogMessagesEnum["WEB_UUID_FOUND"], map[string]interface{}{
			"uuid":    uuid,
			"apiName": apiName,
		}))
	}

	contextModel := user.NewWingifyUserContext(context)
	contextModel.SetUUID(uuid)

	// Create service container
	serviceContainer := core.NewServiceContainer(
		contextModel.ID,
		client.wingifyBuilder.logManager,
		client.wingifyBuilder.settingsManager,
		client.wingifyBuilder.options,
		client.wingifyBuilder.batchEventQueue,
		client.settings,
		client.wingifyBuilder.networkManager,
	)
...
func (client *WingifyClient) TrackEvent(eventName string, context map[string]interface{}, eventProperties ...map[string]interface{}) (result map[string]bool, err error) {
...
// SetAttribute sets multiple attributes for a user and sends an impression to Wingify
func (client *WingifyClient) SetAttribute(attributes map[string]interface{}, context map[string]interface{}) error {
	...
	// Validate attributes
	if len(attributes) == 0 {
		client.wingifyBuilder.logManager.Error("ATTRIBUTES_MAP_ERROR", nil, map[string]interface{}{"an": apiName})
...
// UpdateSettings updates the settings by fetching from the Wingify server
func (client *WingifyClient) UpdateSettings(options ...interface{}) (err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			client.wingifyBuilder.logManager.Error("UPDATING_CLIENT_INSTANCE_FAILED_WHEN_WEBHOOK_TRIGGERED", map[string]interface{}{
				"apiName":      apiName,
	...
	if settings == "" {
		...
		newSettingsData, err := client.wingifyBuilder.settingsManager.FetchSettings(isViaWebhook)
		if err != nil {
			client.wingifyBuilder.logManager.Error("UPDATING_CLIENT_INSTANCE_FAILED_WHEN_WEBHOOK_TRIGGERED", map[string]interface{}{
				"apiName":      apiName,
				"isViaWebhook": isViaWebhook,
	...
	client.wingifyBuilder.originalSettings = newSettings
	client.wingifyBuilder.logManager.Info(log.BuildMessage(log.InfoLogMessagesEnum["SETTINGS_UPDATED"], map[string]interface{}{
		"apiName":      apiName,
		"isViaWebhook": isViaWebhook,
	}))
...
func (client *WingifyClient) GetOriginalSettings() string {
	return client.originalSettings
}

// updateSettingsInternal updates the settings internally (called from polling)
func (client *WingifyClient) updateSettingsInternal(settingsJSON string) (err error) {
	if settingsJSON == "" {
	...
	isSettingsValid, settingsInvalidReason := validateSettingsInternal(settingsJSON, &newSettings, client.wingifyBuilder)
	if !isSettingsValid {
	...
	utils.ProcessSettings(&newSettings, client.wingifyBuilder.logManager)
	return nil
}

// validateSettingsInternal validates the settings using the settings schema (internal function)
func validateSettingsInternal(settingJSON string, processedSettings *settingsModel.Settings, wingifyBuilder *WingifyBuilder) (bool, string) {
	defer func() {
		if r := recover(); r != nil {
			wingifyBuilder.logManager.Error("INVALID_SETTINGS_SCHEMA", map[string]interface{}{
				"errors":    fmt.Sprintf("Error validating settings: %v", r),
				"accountId": strconv.Itoa(wingifyBuilder.options.AccountID),
				"sdkKey":    wingifyBuilder.options.SDKKey,
				"settings":  "null",
			}, map[string]interface{}{"an": enums.ApiUpdateSettings})
		}
	}()
...
func (client *WingifyClient) FlushEvents() (err error) {
	...
	if client.wingifyBuilder.batchEventQueue.IsInitialized() {
		client.wingifyBuilder.batchEventQueue.FlushAndClearInterval()
	} else {
		client.wingifyBuilder.logManager.Error("BATCHING_NOT_ENABLED", nil, map[string]interface{}{"an": apiName})
